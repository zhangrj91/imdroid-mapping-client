 ...
 * @param <S> 参数的类型
 * @author tigerlihao
 * 来源：http://blog.tigerlihao.cn/2010/02/ransac-java.html
 */
public class Ransac<T, S> {
    public List<Point3D> inplanes;
     ...
     *
     * @param paramEstimator           The parameter estimator to be use.
     * @param numForEstimate           The minimal data record number needed to estimate model
     *                                 parameters.
     * @param maximalOutlierPercentage The maximal outlier percentage.
     */
    ...
    public List<Point3D> getinplanes() {
        return inplanes;
    }
     ...
     * @param desiredProbabilityForNoOutliers The probability needed in the estimation.
...
//        if(paramEstimator.getClass().equals(LineParameterEstimator.class)){
//            data = data.subList(0,dataSize/4);
//            dataSize = data.size();
//        }
        ...
        for (int i = 0; i < tryTimes; i++) {
            ...
            for (int j = 0; j < numForEstimate; j++) {
                int selectedIndex;
//                if(paramEstimator.getClass().equals(PlaneParameterEstimator.class))
                selectedIndex = random.nextInt(dataSize - j);
//                else
//                    selectedIndex = t++;
                int k, l;
        ...
        inplanes = (List<Point3D>) leastSqData;
