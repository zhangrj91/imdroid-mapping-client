 ...
 * @author tiger
 */
public class LineParameterEstimator implements ParameterEstimator<Point2D, Double> {
    private double deltaSquared;

    public LineParameterEstimator(double delta) {
        this.deltaSquared = delta * delta;
    }

    /**
     * Is point the point belong to a line.<br>
     * The distance between point PHI(x, y) and line (n_x, n_y, a_x, a_y)<br>
     * can be calculated by the function:<br>
     * d(PHI,l)=(n_x*(p_x-a_x)+n_y*(p_y-a_y))/(n_x*n_x+n_y*n_y)^(1/2)<br>
     *
     * @param params The line parameters (n_x,n_y,a_x,a_y).
     * @param data   The point to be tested.
